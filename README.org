...
| avassa-cli   | Avassa edge computing CLI and supctl device management tool      |
| aws-cli      | AWS CLI v2 with bash completion                                  |
| azure-cli    | Azure CLI with bash/zsh completion (via uv)                      |
| beets        | Music library manager with plugins (via uv)                      |
| bitwarden    | Bitwarden CLI with SSH key management helpers                    |
| bootstrap    | Core system packages and yay/uv installation                     |
| claude-code  | Claude Code agentic AI coding CLI with auto-update               |
...
| logitech     | Logitech device manager (Solaar) with mouse battery monitoring   |
| mcp-servers  | MCP protocol servers for Claude Code (markitdown, context7)      |
| mimeapps     | Default application associations (mpv, zathura, imv, thunar)     |
...
| virt         | QEMU/KVM virtualization with libvirt (virt-manager, OVMF, swtpm) |
