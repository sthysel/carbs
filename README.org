...
CARBS uses Tuckr (a modern alternative to GNU Stow) to manage dotfiles. Tuckr provides hooks, secrets management, and zero-configuration setup.

** Directory Structure

#+begin_example
dotfiles/
├── Configs/     # Your dotfile packages (zsh, nvim, hyprland, etc.)
...
tuckr add -a hyprland           # Adopt (use system version)
...
Create =dotfiles/Hooks/nvim/post.sh=:
...
Make it executable: =chmod +x dotfiles/Hooks/nvim/post.sh=

*** Example: Yazi Hook

Create =dotfiles/Hooks/yazi/post.sh=:
...
git clone https://github.com/youruser/carbs ~/.dotfiles
ln -s ~/.dotfiles/dotfiles ~/.config/dotfiles
...
#+begin_src sh
# Make changes to dotfiles/Configs/hyprland/
...
#+begin_src sh
# 1. Create structure
mkdir -p dotfiles/Configs/btop/.config/btop

# 2. Add config files
cp ~/.config/btop/btop.conf dotfiles/Configs/btop/.config/btop/

# 3. Deploy it
tuckr add btop

# 4. Commit to git
git add dotfiles/Configs/btop
...
1. *Changes are live*: Since files are symlinked, editing =dotfiles/Configs/*/= directly updates your system
2. *Use hooks for automation*: Install plugins, compile configs, restart services automatically
3. *Commit hooks to git*: Share your setup scripts with your future self
4. *Use =-f= carefully*: It overwrites files without backup
5. *Test with =-n=*: Preview changes before applying them
